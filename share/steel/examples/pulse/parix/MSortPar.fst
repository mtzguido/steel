...
let now (f : unit -> stt 'a 'pre 'post) : unit -> stt 'a 'pre 'post = f
...
fn lem1 (e:perm) (p:pool)
  requires promise (pool_done p) (pool_alive #(half_perm e) p)
        ** promise (pool_done p) (pool_alive #(half_perm e) p)
  ensures promise (pool_done p)  (pool_alive #e p)
{
  join_promise 
   #(pool_done p)
...
//   ** promise (pool_done p) (pool_alive #(half_perm e) p)
//   ------------------------------------------------------------------
//   promise (pool_done p) (pool_alive #e p)
//   ** promise (pool_done p) phi

//   q **
//   promise (pool_done p) (pool_alive #(half_perm e) p)
//   ** promise (pool_done p) (pool_alive #(half_perm e) p)
//   ==> promise (pool_done p) (pool_alive #e p ** phi)
...
msort_par
  ...
  (cb : (
    ...
    stt unit (pool_alive #e p ** pts_to_range a lo hi (reveal s))
             (fun () -> promise (pool_done p) (pool_alive #e p) ** pts_to_range a lo hi (sort (reveal s)))
  ))
  (e : perm)
  (lo:nat) (hi : (hi:nat{lo <= hi}))
       ...
       ** pts_to_range a lo hi (sort (reveal s))
{
  if (hi - lo < 2) {
    ...
    return_promise (pool_done p) (pool_alive #e p);
    ()
  } else {
    let mid = (hi+lo) / 2;
    ...
    // now (fun () -> cb lo mid s1 ());
    now (fun () -> cb (half_perm e) mid hi s2) ();
    
    join h1;
    
    lem1 e p;
    // assert (pool_alive #(half_perm e) p ** promise (pool_done p) (pool_alive #(half_perm e) p));

    // make_promise
    //   (pool_done p)
    //   (pool_alive #e p)
    //   (pool_alive #(half_perm e) p ** promise (pool_done p) (pool_alive #(half_perm e) p))
    //   (magic());
